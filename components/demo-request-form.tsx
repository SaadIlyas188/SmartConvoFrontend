...
export function DemoRequestForm({ onClose }: DemoRequestFormProps) {
  ...
  const verifyOtpAndSubmit = async () => {
    ...
    if (enteredOtp === otp) {
      ...
      // ✅ Submit demo request immediately
      setIsSubmitting(true)
      try {
        await emailjs.send("service_u6pur7e", "template_7jjcy5m", {
          company_name: formData.companyName,
  ...
  const resetForm = () => {
    setStep(1)
    setFormData({ companyName: "", email: "", phone: "", emailVerified: false })
    setIsSubmitting(false)
    setShowDemo(false)
    setOtpSent(false)
    setOtp("")
      ...
      >
        ...
        {showDemo ? (
          ...
          <>
            ...
            {step === 2 && (
              <div className="text-center space-y-4">
                <h2 className="text-lg font-semibold">Email Verification & Request Demo</h2>
                {!otpSent ? (
                  <Button onClick={sendOtp} className="mt-3 bg-cyan-500 hover:bg-cyan-400 text-black">
                    Send OTP
                  </Button>
                  ...
                  <div className="space-y-3">
                    <Input
                      ...
                      className="text-center bg-neutral-900 border-neutral-700 text-white"
                    />
                    <Button onClick={verifyOtpAndSubmit} className="bg-green-500 hover:bg-green-600 px-6 text-black">
                      {isSubmitting ? "Preparing Your Demo..." : "Verify & Request Demo"}
                    </Button>
                  </div>
                )}
                {formData.emailVerified && !isSubmitting && (
                  <p className="text-green-400">Email Verified Successfully</p>
                )}
              </div>
            )}
